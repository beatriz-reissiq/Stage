...
function verificarPerfil() {
  ...
  if ((sessaoID, genero == undefined || sessaoID, dataNasc == undefined)) {
...
function publicar() {
  ...
  fetch("/postagens/publicar", {
    ...
    .catch(function (erro) {
      document.getElementById("mensagem_post_error").innerHTML =
  ...
  fetch(`/postagens/listarMeusPosts/${sessionStorage.ID_USUARIO}`)
    ...
    .then(function (posts) {
      ...
      for (let i = 0; i < posts.length; i++) {
        const dataBanco = new Date(posts[i].dataPostagem).toLocaleDateString(
          "pt-BR",
        );
